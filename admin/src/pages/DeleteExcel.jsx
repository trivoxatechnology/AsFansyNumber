...
import { Download, RefreshCw, Check, CircleAlert, Trash2, X, FileText } from 'lucide-react';
...
export default function DeleteExcel() {
  ...
  const { runDeleteOperation } = useImport();
  ...
  const [deleteDestination, setDeleteDestination] = useState('permanent');
  ...
  const onDrop = useCallback(async (files) => {
    ...
    try {
      ...
      const lookupRes = await fetchWithAuth(
        `${API_BASE}/wp_fn_numbers/bulk-lookup`,
        {
          method: 'POST',
          body: JSON.stringify({
            mobile_numbers: validRows.map(r => String(r.mobile_number))
          })
        }
      );
      ...
      const parsed = validRows.map((row, i) => {
        const v = validateDeleteRow(row, i);
        ...
        <div style={s.card}>
          ...
          <div style={s.infoBanner}>
            ...
            <div style={{ fontSize: '0.85rem' }}>
              ...
              The system will match numbers and delete all found entries using High-Speed Bulk SQL.
            </div>
          </div>
          <div {...getRootProps()} style={{ ...s.dropzone, ...(isDragActive ? s.dropActive : {}), borderColor: '#fca5a5' }}>
            ...
            <p style={{ fontWeight: 700, fontSize: '1.1rem', marginBottom: '6px' }}>{isDragActive ? 'Drop here' : 'Drag & Drop or Click to Upload'}</p>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>.xlsx, .xls, .csv</p>
          </div>
          {isParsing && (
            <div style={s.parsingBox}>
              <RefreshCw size={24} style={{ animation: 'spin 1s linear infinite', color: '#ef4444' }} />
              <div>
                <p style={{ fontWeight: 800, marginBottom: '4px', color: 'var(--text-main)' }}>Processing File</p>
      ...
      {step === 2 && (
        <div>
          <div style={s.statsRow}>
            {[{ key: 'all', label: 'Total', count: stats.total, color: '#64748b' }, { key: 'found', label: '🗑 To Delete', count: stats.found, color: '#ef4444' }, { key: 'not_found', label: '⚠ Not Found', count: stats.notFound, color: '#f59e0b' }, { key: 'error', label: '❌ Errors', count: stats.error, color: '#94a3b8' }].map(f => (
              <div key={f.key} onClick={() => setFilter(f.key)} style={{ ...s.statPill, cursor: 'pointer', borderColor: filter === f.key ? f.color : 'var(--border-color)', background: filter === f.key ? f.color + '10' : 'var(--bg-card)', transition: 'all 0.2s' }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <button onClick={() => setStep(3)} style={{ ...s.primaryBtn, background: '#ef4444' }} disabled={stats.found === 0}>Next Step →</button>
            </div>
          </div>
          {display.length === 0 ? <div style={{ padding: '60px', textAlign: 'center', background: 'var(--bg-card)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', color: 'var(--text-muted)' }}>No rows found.</div> : (
            <>
              <div style={s.gridWrap}>
                <table style={s.table}>
                  <thead><tr>{['Row', 'Status', 'Mobile Number', 'DB Match'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
                  <tbody>{display.slice(0, displayLimit).map((row, i) => (
                    <tr key={i} style={{ background: row._found ? '#fef2f2' : row._status === 'not_found' ? '#fffbeb' : 'transparent' }}>
                      ...
                      <td style={s.td}><span style={{ ...s.badge, background: row._found ? '#fee2e2' : row._status === 'not_found' ? '#fef3c7' : '#f1f5f9', color: row._found ? '#dc2626' : row._status === 'not_found' ? '#d97706' : '#94a3b8' }}>{row._found ? '🗑 DELETE' : row._status === 'not_found' ? '⚠ NOT FOUND' : '❌ ERROR'}</span></td>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', background: '#f8fafc', borderBottomLeftRadius: 'var(--radius-md)', borderBottomRightRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', borderTop: 'none' }}>
                <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: 600 }}>Showing {Math.min(displayLimit, display.length).toLocaleString()} rows</span>
                {display.length > displayLimit && <button onClick={() => setDisplayLimit(p => p + 500)} style={s.outlineBtn}>Load Next 500</button>}
              </div>
            </>
          )}
        </div>
        ...
        <div style={s.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
            <div>
              ...
              <p style={s.cardSubtitle}>Confirm where to move these found entries.</p>
            </div>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowConfirmModal(true)} style={{ ...s.primaryBtn, background: '#ef4444' }}>Choice Destination & Confirm</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999,backdropFilter:'blur(4px)'}}>
          <div style={{background:'var(--bg-card)',width:'100%',maxWidth:'550px',borderRadius:'var(--radius-lg)',boxShadow:'0 20px 25px -5px rgba(0,0,0,0.1)',overflow:'hidden'}}>
            <div style={{padding:'24px',borderBottom:'1px solid var(--border-color)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <button onClick={()=>setShowConfirmModal(false)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-muted)'}}><X size={20}/></button>
            </div>
            <div style={{padding:'24px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',marginBottom:'24px'}}>
                <div onClick={()=>{setDeleteDestination('draft')}} style={{border:`2px solid ${deleteDestination==='draft'?'#f59e0b':'var(--border-color)'}`,borderRadius:'var(--radius-md)',padding:'16px',cursor:'pointer',background:deleteDestination==='draft'?'#fffbeb':'transparent',textAlign:'center'}}>
                  <div style={{width:'40px',height:'40px',borderRadius:'50%',background:deleteDestination==='draft'?'#fef3c7':'#f1f5f9',margin:'0 auto 12px',display:'flex',alignItems:'center',justifyContent:'center',color:deleteDestination==='draft'?'#d97706':'#64748b'}}><FileText size={20}/></div>
                  <h4 style={{margin:0,fontWeight:800,color:'var(--text-main)',marginBottom:'4px'}}>Move to Drafts</h4>
                  <p style={{margin:0,fontSize:'0.8rem',color:'var(--text-muted)'}}>Soft-delete. Recoverable.</p>
                </div>
                <div onClick={()=>{setDeleteDestination('permanent')}} style={{border:`2px solid ${deleteDestination==='permanent'?'#ef4444':'var(--border-color)'}`,borderRadius:'var(--radius-md)',padding:'16px',cursor:'pointer',background:deleteDestination==='permanent'?'#fef2f2':'transparent',textAlign:'center'}}>
                  <div style={{width:'40px',height:'40px',borderRadius:'50%',background:deleteDestination==='permanent'?'#fee2e2':'#f1f5f9',margin:'0 auto 12px',display:'flex',alignItems:'center',justifyContent:'center',color:deleteDestination==='permanent'?'#dc2626':'#64748b'}}><Trash2 size={20}/></div>
                  <h4 style={{margin:0,fontWeight:800,color:'var(--text-main)',marginBottom:'4px'}}>Permanent Delete</h4>
                  <p style={{margin:0,fontSize:'0.8rem',color:'var(--text-muted)'}}>Irreversible action.</p>
                </div>
              </div>
              <div style={{background:'#f8fafc',padding:'16px',borderRadius:'var(--radius-md)',border:'1px solid var(--border-color)'}}>
                ...
                <input type="text" value={operatorName} onChange={e => setOperatorName(e.target.value)} style={{padding:'10px 14px',borderRadius:'8px',border:'1px solid var(--border-color)',width:'100%',outline:'none',fontWeight:600}} />
              </div>
            </div>
            <div style={{padding:'16px 24px',background:'#f8fafc',borderTop:'1px solid var(--border-color)',display:'flex',justifyContent:'flex-end',gap:'12px'}}>
              <button onClick={()=>{setShowConfirmModal(false)}} style={s.outlineBtn}>Cancel</button>
              <button onClick={handleDelete} style={{...s.primaryBtn, background:'#ef4444', minWidth:'140px', justifyContent:'center'}}><Trash2 size={16}/> {deleteDestination === 'permanent' ? 'Permanently Delete' : 'Move to Drafts'}</button>
            </div>
          </div>
        </div>
      )}
...
const s = {
  ...
  step: { display: 'flex', alignItems: 'center', gap: '8px', flex: 1, opacity: 0.4 },
  stepActive: { opacity: 1 },
  stepCircle: { width: '26px', height: '26px', borderRadius: '50%', background: '#ef4444', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.78rem' },
  stepLabel: { fontSize: '0.82rem', fontWeight: 700 },
  ...
  code: { background: '#fee2e2', padding: '2px 6px', borderRadius: '4px', fontWeight: 700 },
  ...
  statsRow: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' },
  ...
  th: { background: '#fef2f2', padding: '10px 14px', borderBottom: '1px solid var(--border-color)', color: '#ef4444', fontWeight: 700, textAlign: 'left' },
