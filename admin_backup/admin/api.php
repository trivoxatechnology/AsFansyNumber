    ...
    try {
        ...
        $stmt = $pdo->query("
            SELECT
                ...
                SUM(CASE WHEN `number_status` = 'sold' AND `sold_at` >= '$thisMonth' THEN 1 ELSE 0 END) as sold_this_month,
                SUM(CASE WHEN `number_status` = 'sold' AND `sold_at` >= '$thisMonth' THEN `base_price` ELSE 0 END) as revenue_this_month
            FROM `wp_fn_numbers`
        ");
        $result = $stmt->fetch();
...
/**
 * GET /wp_fn_numbers/pattern-stats
 */
function handle_pattern_stats($pdo) {
    try {
        $stmt = $pdo->query("
            SELECT 
                `category`,
            ...
            FROM `wp_fn_numbers`
            GROUP BY `category`
            ORDER BY FIELD(`category`, 'Diamond', 'Platinum', 'Gold', 'Silver', 'Normal')
        ");
        echo json_encode(["success" => true, "data" => $stmt->fetchAll()]);
    } catch (Exception $e) {
        http_response_code(500);
 ...
 */
function handle_migrate($pdo, $table) {
    if (!in_array($table, ['wp_fn_numbers', 'wp_fn_draft_numbers'])) {
        echo json_encode(["error" => "Migration not supported for this table"]);
        return;
    }

    $columnsToAdd = [
        'primary_incharge_name'   => "VARCHAR(255) NULL",
        'primary_incharge_phone'  => "VARCHAR(20) NULL",
        'secondary_incharge_name' => "VARCHAR(255) NULL",
        'secondary_incharge_phone'=> "VARCHAR(20) NULL",
        'whatsapp_group_name'     => "VARCHAR(255) NULL",
        'offer_start_date'        => "DATETIME NULL",
        'offer_end_date'          => "DATETIME NULL",
        'priority_rank'           => "INT(11) DEFAULT 0",
        'digit_sum'               => "INT(11) DEFAULT 0",
        'repeat_count'            => "INT(11) DEFAULT 0",
        'prefix'                  => "VARCHAR(10) NULL",
        'suffix'                  => "VARCHAR(10) NULL",
        'category_name'           => "VARCHAR(255) NULL",
        'pattern_name'            => "VARCHAR(255) NULL",
        'vip_score'               => "INT(11) DEFAULT 0",
        'number_category'         => "VARCHAR(100) NULL",
        'discount_percentage'     => "DECIMAL(5,2) DEFAULT 0.00",
        'offer_tag'               => "VARCHAR(100) NULL",
    ...
    try {
        ...
        foreach ($columnsToAdd as $col => $definition) {
            if (!in_array($col, $existingCols)) {
                ...
                $log[] = "Column already exists: $col";
            }
        }

        echo json_encode(["success" => true, "table" => $table, "changes" => $log]);
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(["success" => false, "error" => $e->getMessage()]);
    }
...
function handle_bulk_insert($pdo, $target, $input) {
    ...
    // NEW: Schema-Awareness — Fetch actual columns once to prevent crashes
...
function handle_bulk_update($pdo, $target, $input) {
    ...
    $allowed = [
        ...
        'offer_tag', 'inventory_source', 'number_type'
    ];
...
function build_field_list($table) {
    $tableFields = [
        'wp_fn_numbers' => [
            ...
            'visibility_status', 'remarks', 'created_at', 'updated_at'
        ],
        'wp_fn_draft_numbers' => [
            'number_id', 'mobile_number', 'base_price', 'offer_price',
            'number_status', 'category', 'pattern_type',
            'inventory_source', 'visibility_status', 'remarks', 'created_at', 'updated_at'
        ],
        'wp_fn_background_jobs' => [
